...
public class SwaggerConfig {
	...
	@Bean
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select().paths(PathSelectors.ant("/products/**"))
				.apis(RequestHandlerSelectors.basePackage("com.example.demo")).build().apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		return new ApiInfo("User API Service", // Title
				"The Api handles the basic CRUD operation for user management", // Description
				"Version 1.1", // Version
				"My Terms of Service", new Contact("Ms krithiga", "krithigaa@gmail.com", "kiki@outlook.com"),
