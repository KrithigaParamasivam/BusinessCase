...
const { setValues,handleSubmit,handleChange,values,errors} = useFormik({
  ...
  validationSchema,
  onSubmit(values) {
    axios.put("http://localhost:8080/products/update",values).then(res=>res.data).then((data)=>{
    ...
    return (
        <div>           
            ...
            <div className="card1"> 
            <div className="card-header"><h4>Edit PRODUCTS</h4></div>
  ...
  <button class="btn btn-primary" type="submit" onClick={handleSubmit} >Update</button>
