...
export default function PendingDelivery() {
  ...
  const validationSchema = Yup.object({
    ...
    customerPhoneno: Yup.string().required("Customer Phone number is mandatory"),
...
const { setValues,handleSubmit,handleChange,values,errors} = useFormik({
  ...
  validationSchema,
  onSubmit(values) {
    axios.post("http://localhost:8080/addpending",values).then(res=>res.data).then((data)=>{
        ...
        axios.put("http://localhost:8080/update",values).then(res=>res.data).then((data)=>{
          //alert("User Onboarded successfully");
          console.log("=============Submitted");
          console.log(data);
          swal({
    ...
    return (
        <div>           
            ...
            <div className="card"> 
            <div className="card-header"><h4>ADD PENDING PRODUCTS</h4></div>
